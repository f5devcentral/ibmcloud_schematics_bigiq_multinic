...
def get_public_images(region):
    if not region:
        region = "us-south"
    catalog_url = (
        "https://f5-mgmt-%s.s3.%s.cloud-object-storage.appdomain.cloud/f5-image-catalog.json"
        % (region, region)
    )
    try:
        ...
        sys.stderr.write("Can not fetch F5 image catalog at %s: %s" % (catalog_url, ex))
        sys.exit(1)
...
def get_image(region, version):
    ...
    for image in image_catalog[region]:
        if not image["image_id"]:
            continue
        match_length = longest_substr(image["image_name"], version)
...
def main():
    ...
    if "region" not in jsondata:
        sys.stderr.write(
            "region, verion_prefix inputs require to query public f5 images"
        )
        sys.exit(1)
    ...
    (image_url, image_name, image_id) = get_image(region, bigiq_version_match)
